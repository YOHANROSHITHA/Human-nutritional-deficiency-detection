  ...
  try {
    ...
    const htmlContent = `
      <div style="font-family: sans-serif; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 10px;">
        <h2 style="color: #00cccc;">NutriGuard AI - Diagnostic Report</h2>
        <p>Hello,</p>
        <p>Here is your nutritional deficiency analysis report:</p>
        <div style="background: #f9f9f9; padding: 15px; border-radius: 8px; margin: 20px 0;">
          <h3 style="margin-top: 0;">Identified Marker: <span style="color: #ff4d4d;">${result.deficiency}</span></h3>
          <p><strong>Confidence Score:</strong> ${result.confidence}</p>
          <p><strong>Clinical Advice:</strong> ${result.advice}</p>
        </div>
        <h4>Recommended Foods:</h4>
        <ul style="color: #555;">${result.foods.map(f => `<li>${f}</li>`).join('')}</ul>
        <p style="font-size: 12px; color: #888; margin-top: 30px; border-top: 1px solid #eee; pt: 10px;">
          Disclaimer: This report is generated by an AI system and is for informational purposes only. Please consult a doctor for medical advice.
        </p>
      </div>
    `;

    const info = await transporter.sendMail({
      from: '"NutriGuard AI" <reports@nutriguard.ai>',
      to: email,
      subject: `Your Nutritional Report: ${result.deficiency}`,
      html: htmlContent,
    });

    return res.json({ message: 'Report sent successfully!', previewUrl: nodemailer.getTestMessageUrl(info) });
  } catch (error) {
